...
QString VerovioEngine::renderAllPagesHtml() const
{
    ...
    html += QStringLiteral("\n"
        ...
        "function setMeasureTicks(ticks) {\n"
        "  // Merge measure-start positions into the note timemap to fill silent-measure gaps\n"
        "  var measures = document.querySelectorAll('.measure');\n"
        "  var n = Math.min(ticks.length, measures.length);\n"
        "  // Build a set of ticks that already have note entries (within 20 ticks tolerance)\n"
        "  var noteTicks = new Set();\n"
        "  _timemap.forEach(function(e) { noteTicks.add(Math.round(e.tick / 20) * 20); });\n"
        "  // Add measure entries only where no note exists\n"
        "  for (var i = 0; i < n; i++) {\n"
        "    var roundedTick = Math.round(ticks[i] / 20) * 20;\n"
        "    if (noteTicks.has(roundedTick)) continue;\n"
        "    var r = measures[i].getBoundingClientRect();\n"
        "    if (r.width <= 0) continue;\n"
        "    // Create a virtual element-like ID for this measure position\n"
        "    var vid = '_m' + i;\n"
        "    var div = document.createElement('div');\n"
        "    div.id = vid;\n"
        "    div.style.cssText = 'position:absolute;left:' + (r.left + window.scrollX + 4) + 'px;top:'\n"
        "      + (r.top + window.scrollY) + 'px;width:1px;height:1px;';\n"
        "    document.body.appendChild(div);\n"
        "    _timemap.push({tick: ticks[i], id: vid});\n"
        "  }\n"
        "  _timemap.sort(function(a,b) { return a.tick - b.tick; });\n"
        "}\n"
        ...
        "  });\n"
        "}, 300);\n"
        "// Push scroll offset to Qt overlay on every scroll\n"
        "window.addEventListener('scroll', function() {\n"
        "  if (window._qtScrollCallback) window._qtScrollCallback(window.scrollY);\n"
        "}, {passive:true});\n"
        "function _getSysBounds(sys) {\n"
        "  // Compute full vertical extent of the system from all child elements\n"
        "  var minY = Infinity, maxY = -Infinity;\n"
        "  sys.querySelectorAll('.staff, .note, .rest, .staffLines').forEach(function(el) {\n"
        ...
        "  if (minY < maxY) {\n"
        "    var pad = 6;\n"
        "    return {top: minY + window.scrollY - pad, height: (maxY - minY) + pad * 2};\n"
        "  }\n"
        "  var sr = sys.getBoundingClientRect();\n"
        "  return {top: sr.top + window.scrollY, height: sr.height};\n"
        "}\n"
        ...
        "      if (s1 && s2 && s1 === s2 && dt > 0) {\n"
        "        // Same system: interpolate between element positions\n"
        ...
        "        var r2 = e2.getBoundingClientRect();\n"
        "        var noteX2 = r2.left + r2.width/2;\n"
        "        var firstEl2 = s2.querySelector('.note, .rest, .mRest');\n"
        "        var startX2 = firstEl2 ? firstEl2.getBoundingClientRect().left : (s2.getBoundingClientRect().left + 10);\n"
        "        var distEnd = rightEdge - x;\n"
        "        var distStart = noteX2 - startX2;\n"
        "        var split = (distEnd + distStart > 0) ? distEnd / (distEnd + distStart) : 0.5;\n"
        "        if (f < split) {\n"
        "          x += (rightEdge - x) * (f / split);\n"
        "        } else {\n"
        "          s1 = s2;\n"
        "          var f2 = (f - split) / (1 - split);\n"
        ...
        "  _cursorX = x;\n"
        "  _cursorSysTop = sb.top;\n"
        "  _cursorSysBottom = sb.top + sb.height;\n"
        ...
        "var _cursorX = 0;\n"
        "var _cursorSysTop = 0;\n"
        "var _cursorSysBottom = 0;\n"
        "function getCursorX() { return _cursorX; }\n"
        "function getCursorSysBounds() { return [_cursorSysTop, _cursorSysBottom]; }\n"
        "function hideCursor() { if (_cursorEl) _cursorEl.style.display='none'; }\n"
        "var _clickedNoteId = '';\n"
        "function getClickedNote() { var id = _clickedNoteId; _clickedNoteId = ''; return id; }\n"
        "document.addEventListener('click', function(e) {\n"
        "  var el = e.target.closest('.note');\n"
        "  if (el && el.id) { _clickedNoteId = el.id; return; }\n"
        "  // No direct hit — find nearest note within 30px\n"
        ...
        "    var d = Math.sqrt((cx-nx)*(cx-nx) + (cy-ny)*(cy-ny));\n"
        "    if (d < bestD) { bestD = d; best = n; }\n"
        "  });\n"
        "  if (best && best.id) { _clickedNoteId = best.id; }\n"
        "});\n"
        "</script>\n"
        "</head>\n<body>\n");
