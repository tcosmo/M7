...
QString VerovioEngine::renderAllPagesHtml() const
{
    ...
    html += QStringLiteral(
        ...
        "    style:'fill:rgba(50,120,255,0.22);stroke:rgb(50,120,255);stroke-width:45'},\n"
        "  'highlight-v1': {rc:'hl-v1',\n"
        "    style:'fill:rgba(180,80,220,0.22);stroke:rgb(180,80,220);stroke-width:45'}\n"
        "};\n"
        ...
        "          x += (rightEdge - x) * (f * 2);\n"
        "        } else {\n"
        "          // Second half: in new system, interpolate from left edge toward first note\n"
        "          var r2 = e2.getBoundingClientRect();\n"
        "          var leftEdge = s2.getBoundingClientRect().left + 10;\n"
        "          s1 = s2; // switch cursor to new system\n"
        "          x = leftEdge + (r2.left + r2.width/2 - leftEdge) * ((f - 0.5) * 2);\n"
        "        }\n"
        "      }\n"
        "    }\n"
        "  }\n"
