...
QString VerovioEngine::renderAllPagesHtml() const
{
    ...
    html += QStringLiteral("\n"
        ...
        "          x += (rightEdge - x) * (f * 2);\n"
        "        } else {\n"
        "          // Second half: entering new system.\n"
        "          // Interpolate from first musical element toward first note.\n"
        "          s1 = s2;\n"
        "          var r2 = e2.getBoundingClientRect();\n"
        "          var noteX = r2.left + r2.width/2;\n"
        "          var firstEl = s2.querySelector('.note, .rest');\n"
        "          var startX = firstEl ? firstEl.getBoundingClientRect().left : noteX;\n"
        "          var f2 = (f - 0.5) * 2;\n"
